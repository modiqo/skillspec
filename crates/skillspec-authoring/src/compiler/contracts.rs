...
pub(super) fn write_runtime_contract(output: &mut String) {
    ...
    output.push_str("- Keep SkillSpec implementation details out of normal user-facing progress. Do not narrate ledger rows, raw progress commands, checkpoint rows, trace plumbing, or alignment internals. Show plain-language status only: assessed, planned, changed, verified, blocked, or complete.\n");
    ...
    output.push_str("- Do not run `skillspec progress ... --help` or query `command:progress_*` during normal execution; the guide provides the checkpoint flag shape and quiet command. Use help/query only for debugging or when a blocker proves the local CLI contract is unclear.\n");
