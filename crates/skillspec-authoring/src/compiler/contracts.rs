...
pub(super) fn write_runtime_contract(output: &mut String) {
    ...
    output.push_str("- Keep SkillSpec implementation details out of normal user-facing progress. Do not narrate ledger rows, raw progress commands, evidence-batch JSONL rows, trace plumbing, or alignment internals. Show plain-language status only: assessed, planned, changed, verified, blocked, or complete.\n");
    output.push_str("- For read-only diagnostic routes, including Doctor/source-shape assessment, answer from the diagnostic output and stop. Do not create source maps, import drafts, progress ledgers, final-response proof, or alignment summaries unless the user explicitly asks for proof.\n");
    output.push_str("- Do not run `skillspec progress ... --help` or query `command:progress_*` during normal execution; the guide provides the JSONL row shape and quiet batch command. Use help/query only for debugging or when a blocker proves the local CLI contract is unclear.\n");
    ...
    output.push_str("- Alignment proof rows may be batched through `<run_dir>/final-proof.jsonl`; use the grouped proof digest as a checklist and record real evidence once with `skillspec progress batch --file <run_dir>/final-proof.jsonl --checkpoint \"checkpointing evidence\" --quiet`.\n");
    ...
    output.push_str("- `Alignment summary`: include alignment status or the alignment report path. Include detailed `Decision replay`, `Phase order`, `Requirements`, `Missing proof`, and `Forbidden actions` rows only when debugging, blocked, failed, or explicitly requested.\n");
    output.push_str("- `Token usage`: include measured `Token consumption` and `Token savings` from alignment artifacts when available; otherwise include estimated summary metrics or say `not recorded`.\n");
