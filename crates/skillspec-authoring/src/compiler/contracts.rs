...
pub(super) fn write_runtime_contract(output: &mut String) {
    ...
    output.push_str("- Keep SkillSpec implementation details out of normal user-facing progress. Do not narrate ledger rows, raw progress commands, trace plumbing, or alignment internals. Show plain-language status only: assessed, planned, changed, verified, blocked, or complete.\n");
    ...
    output.push_str("- Alignment proof rows may be batched through `<run_dir>/final-proof.jsonl`; use the grouped proof digest as a checklist and record real evidence once with `skillspec progress batch --file <run_dir>/final-proof.jsonl --checkpoint \"checkpointing evidence\" --summary`.\n");
