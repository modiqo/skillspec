...
pub(super) fn write_runtime_contract(output: &mut String) {
    ...
    output.push_str("- Token economy applies even without Rote: prefer compact CLI summaries, source-map handles, query/ref summaries, alignment rows, and artifact paths; load full reports or full source spans only when exact evidence is required.\n");
    ...
    output.push_str("- `Token usage`: include measured `Token consumption` and `Token savings` from `skillspec trace align` when available; otherwise include estimated non-rote summary metrics or say `not recorded`.\n");
