...
description: Primary implicit skill-discovery entry point for a SkillSpec-indexed local skill library. Use first to choose the right installed skill when router mode is enabled, especially when many skills are installed or descriptions may be truncated.
metadata:
  routing:
...
the primary implicit discovery entry point for the managed skill roots. All
routed skills are explicit-only/manual-only, so choose a domain skill through
the router index first, then load the selected skill explicitly.
