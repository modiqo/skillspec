    ...
    execution_plan:
      ...
      phases:
        - id: route_query
          ...
          description: Query the router index once, inspect decision, selected, bypass_reason, execution_policy, and candidates, and load a skill only when decision is use_skill.
          requires:
            - run_route_query
        - id: match_gate
          owner_skill: {router_skill}
          description: If decision is use_skill and selected is non-null, load that selected skill explicitly and obey execution_policy forbids/preferences. If decision is bypass, continue normal agent behavior unless execution_policy reports unavailable required substrate, in which case report repair guidance. If decision is ambiguous, do not silently load a candidate; ask only when the user explicitly wanted skill selection, otherwise continue normal behavior.
          requires:
...
commands:
  ...
  run_route_query:
    description: Route the user request to candidate skills from the index. Duplicate physical roots collapse to one logical skill before matching; optional current harness/root context only chooses the installed copy. The JSON decision is authoritative; use_skill loads selected, bypass continues normal behavior, and ambiguous must not silently load a candidate. execution_policy is the step ladder from selected skill to rote substrate; service_api uses rote_adapter, local_action uses rote_shell, and browse uses rote_browse when active.
    template: {route_command}
    safety: local_read

  apply_route_decision:
    description: Apply route JSON without inventing a match. Load selected only for decision use_skill. For bypass, continue normal agent behavior unless execution_policy is unavailable and supplies repair. For ambiguous, ask only when the user explicitly requested skill choice; otherwise continue normal behavior. When execution_policy is active, obey its substrate, forbids, and preferences.
    template: 'inspect route JSON fields: decision, selected, bypass_reason, execution_policy, candidates'
    safety: local_read
