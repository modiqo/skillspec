...
| 02 | [Runtime Trampoline And Alignment](02-runtime-trampoline-and-alignment.md) | How the thin `SKILL.md` loader keeps the agent inside `skill.spec.yml`, then uses `plan`, `act`, `progress`, and `trace align` as an OODA loop. |
| 03 | [Router Mode](03-router-mode.md) | How router mode controls skill explosion by making skills explicit-only, building an index, and preserving durable-executor as the implicit exception. |
