...
| `skillspec run-loop` | `<path>`, `--input <text>`, `--view index,summary,full`, `--trace-dir <dir>`, `--phase <id>`, `--json` | Batches sensemake, decide, plan, and the first or requested action checklist in one spec load. This reduces repeated CLI invocations and repeated spec parsing for the common planning loop; it does not execute tools. | `skillspec run-loop ./skill.spec.yml --input 'port this skill' --view index` |
...
| `skillspec trace align` | `<path>`, `--decision-trace <run-dir>`, `--execution-trace <jsonl>`, `--summary`, `--json` | Replays the decision trace against the current spec and checks structured execution evidence for obligations. `--summary` prints only the completion-facing alignment/token block while writing full detail to `<run-dir>/alignment.json`. | `skillspec trace align ./skill.spec.yml --decision-trace .skillspec/traces/run-123 --execution-trace .skillspec/traces/run-123/execution.jsonl --summary` |
...
| `skillspec source map` | `<path>`, `--out <dir>`, `--json` | Builds `source-map.json` and `source-map.md` from a prose skill file or folder using Markdown AST positions for normal files and a chunked heading/code/paragraph mapper for oversized Markdown. Use it before importing large or resource-heavy skills so the agent can query exact source spans instead of loading the whole source. | `skillspec source map ./source-skill --out ./draft/.skillspec/source-map` |
...
| `skillspec doctor` | `<target>`, `--json` | Static shape gate plus single-skill diagnostic for local folders, public GitHub skill folders, and public GitHub repo URIs. Reports `simple_skill`, `entry_skill_with_subskills`, `multi_skill_workspace`, `plugin_workspace`, or `non_skill_repository`; only `simple_skill` continues to full structural scoring. Shape-only reports recommend the next command instead of wasting work on parent workspaces or code repos. Remote targets are staged through a temporary partial sparse checkout and cleaned up after the report. | `skillspec doctor https://github.com/owner/repo --json` |
