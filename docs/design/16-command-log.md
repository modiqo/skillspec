...
| `skillspec workspace map` | `<source-root>`, `--out <skillspec.workspace.yml>`, `--json` | Authoring-side structure recon for multi-skill repositories. Discovers folders with `SKILL.md`, parses public names and visibility, assigns package ids and deterministic install slugs, scans Markdown for cross-package references, infers `depends_on` edges, reports duplicate public names/install slugs, and writes a manifest plus markdown report. This is not router indexing. | `skillspec workspace map ./skills --out ./build/skillspec.workspace.yml` |
| `skillspec workspace validate` | `<skillspec.workspace.yml>`, `--json` | Validates a workspace package graph before fanout import. Checks source root, package paths, exactly one `SKILL.md` per package, dependency references, self-dependencies, cycles, install slug uniqueness, and uncovered cross-package references. Duplicate public names are warnings until install planning. | `skillspec workspace validate ./build/skillspec.workspace.yml` |
