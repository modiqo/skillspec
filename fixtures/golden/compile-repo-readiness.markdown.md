...
- For each execution phase, run `skillspec act <skill-folder>/skill.spec.yml --input='<task>' --run <run_dir> --phase <phase-id>` before acting, obey the rendered phase tool boundary, then record routine successful phase/requirement evidence with one quiet `skillspec progress checkpoint ... --quiet` command at natural boundaries. Use `skillspec progress show <skill-folder>/skill.spec.yml --run <run_dir> --quiet` as an internal gate check for completed, current, blocked, and remaining phases. Surface only a plain-language gate result when useful, unless the user asks for details or a blocker/failure needs evidence.
...
- Keep SkillSpec implementation details out of normal user-facing progress. Do not narrate ledger rows, raw progress commands, checkpoint rows, trace plumbing, or alignment internals. Show plain-language status only: assessed, planned, changed, verified, blocked, or complete.
...
- Do not run `skillspec progress ... --help` or query `command:progress_*` during normal execution; the guide provides the checkpoint flag shape and quiet command. Use help/query only for debugging or when a blocker proves the local CLI contract is unclear.
...
- Missing alignment proof means partial alignment unless the evidence was captured when the work happened. Do not create final-proof rows after the fact to satisfy route, obligation, elicitation, or phase requirements. Use progress checkpoint or batch only for real phase-boundary evidence that already exists.
...
# routine success evidence: checkpoint typed rows quietly once, without hand-authoring JSONL
skillspec progress checkpoint "${PWD}/.skillspec/traces/<run-id>" --requirement-satisfied <phase-id>/<requirement-id>=<kind>:<ref> --phase-completed <phase-id>=<kind>:<ref> --checkpoint "checkpointing evidence" --quiet
