...
- For each execution phase, run `skillspec act <skill-folder>/skill.spec.yml --input='<task>' --run <run_dir> --phase <phase-id>` before acting, obey the rendered phase tool boundary, stage routine successful phase/requirement evidence in a JSONL batch without displaying the rows, then run one quiet `skillspec progress batch ... --quiet` checkpoint at natural boundaries. Use `skillspec progress show <skill-folder>/skill.spec.yml --run <run_dir> --quiet` as an internal gate check for completed, current, blocked, and remaining phases. Surface only a plain-language gate result when useful, unless the user asks for details or a blocker/failure needs evidence.
...
- Token economy applies to direct SkillSpec CLI runs: prefer compact CLI summaries, source-map handles, query/ref summaries, alignment rows, and artifact paths; load full reports or full source spans only when exact evidence is required.
...
- When the CLI is available, run `skillspec trace align <skill-folder>/skill.spec.yml --decision-trace <run_dir> --quiet --proof-digest <run_dir>/proof-digest.json` and add `--execution-trace <run_dir>/execution.jsonl` when structured action evidence exists. This writes `<run_dir>/alignment.json` and a grouped proof digest without transcript output. Include only the alignment status or report path, token status, and trace path unless debugging, failure, or user request requires detailed checks. Do not report a bare `unproven`; use `Alignment: partial` plus specific `Missing proof` rows only when those rows are already known or explicitly inspected.
- Keep SkillSpec implementation details out of normal user-facing progress. Do not narrate ledger rows, raw progress commands, evidence-batch JSONL rows, trace plumbing, or alignment internals. Show plain-language status only: assessed, planned, changed, verified, blocked, or complete.
- For read-only diagnostic routes, including Doctor/source-shape assessment, answer from the diagnostic output and stop. Do not create source maps, import drafts, progress ledgers, final-response proof, or alignment summaries unless the user explicitly asks for proof.
- Do not run `skillspec progress ... --help` or query `command:progress_*` during normal execution; the guide provides the JSONL row shape and quiet batch command. Use help/query only for debugging or when a blocker proves the local CLI contract is unclear.
- For proof-bearing routes, include token usage in the completion report when recorded. For durable-executor runs, collect durable workspace stats into a report file and run `skillspec progress stats <run_dir> --workspace <workspace> --workspace-stats-report <file> --phase <phase-id> --requirement <stats-requirement-id> --quiet` before alignment; missing `stats_collected` evidence is a workflow bug, not a normal omission. Draft the final response with Result, Evidence, Alignment status/report path, Token usage, and SkillSpec sections, run one initial `skillspec trace align --quiet --proof-digest <run_dir>/proof-digest.json`, batch any real missing proof rows into `<run_dir>/final-proof.jsonl`, run `skillspec progress batch <run_dir> --file <run_dir>/final-proof.jsonl --checkpoint "checkpointing evidence" --quiet` once if there are multiple rows, run `skillspec progress final-response <run_dir> --phase <phase-id> --requirement <report-requirement-id> --result --evidence --alignment --token-savings --quiet`, then rerun `skillspec trace align --quiet` once and report the final result in plain language. Do not rerun alignment after each individual progress row. Use measured `Token consumption` and `Token savings` from alignment artifacts when available; if stats truly cannot be collected, say `not recorded`. For direct CLI runs, record `--summary` metrics first with `skillspec progress stats <run_dir> --agent-visible-tokens <n> --artifact-tokens-preserved <n> --avoided-tokens <n> --metrics-source estimated --quiet`, then report them as estimated output economy, not measured model usage. In direct runs, the Evidence section should name trace files and artifacts only; do not mention durable-executor or its underlying tools unless that route was actually used. Do not invent replay savings.
- Alignment proof rows may be batched through `<run_dir>/final-proof.jsonl`; use the grouped proof digest as a checklist and record real evidence once with `skillspec progress batch --file <run_dir>/final-proof.jsonl --checkpoint "checkpointing evidence" --quiet`.
...
- `Evidence`: for durable-executor runs, workspace name plus important response ids/files the user can query later; for direct CLI runs, trace and artifact paths only.
- `Alignment summary`: include alignment status or the alignment report path. Include detailed `Decision replay`, `Phase order`, `Requirements`, `Missing proof`, and `Forbidden actions` rows only when debugging, blocked, failed, or explicitly requested.
- `Token usage`: include measured `Token consumption` and `Token savings` from alignment artifacts when available; otherwise include estimated summary metrics or say `not recorded`.
...
- For one atomic prose skill, prefer `skillspec port-one-shot <source> --out <draft> --target codex-skill --prove` before hand-editing. It writes grammar/schema proof, a typed shape crib, source map, doctor report, mechanical draft, QA results, compile output, and optional estimated summary stats when `--run-dir` is supplied.
...
# routine success evidence: append JSONL rows to evidence-batch.jsonl, then checkpoint quietly once
skillspec progress batch "${PWD}/.skillspec/traces/<run-id>" --file "${PWD}/.skillspec/traces/<run-id>/evidence-batch.jsonl" --checkpoint "checkpointing evidence" --quiet
# troubleshooting/failures only: record an individual row when a blocker needs exact foreground evidence
skillspec progress record "${PWD}/.skillspec/traces/<run-id>" phase-completed <phase-id> --evidence-kind <kind> --evidence-ref <ref>
skillspec progress stats "${PWD}/.skillspec/traces/<run-id>" --workspace <workspace> --workspace-stats-report "${PWD}/.skillspec/traces/<run-id>/workspace-stats.txt" --phase <phase-id> --requirement <stats-requirement-id> --quiet
skillspec progress stats "${PWD}/.skillspec/traces/<run-id>" --agent-visible-tokens <n> --artifact-tokens-preserved <n> --avoided-tokens <n> --metrics-source estimated --phase <phase-id> --requirement <stats-requirement-id> --quiet
skillspec progress final-response "${PWD}/.skillspec/traces/<run-id>" --phase <phase-id> --requirement <report-requirement-id> --result --evidence --alignment --token-savings --quiet
skillspec progress batch "${PWD}/.skillspec/traces/<run-id>" --file "${PWD}/.skillspec/traces/<run-id>/final-proof.jsonl" --checkpoint "checkpointing evidence" --quiet
...
skillspec trace align <skill-folder>/skill.spec.yml --decision-trace "${PWD}/.skillspec/traces/<run-id>" --execution-trace "${PWD}/.skillspec/traces/<run-id>/execution.jsonl" --proof-digest "${PWD}/.skillspec/traces/<run-id>/proof-digest.json" --quiet
```
